...
export default function PickupPage(){
  ...
  const handleSubmit = (e) => {
    ...
    const formData = {
      pickupType,
      wasteVolume: getVolumeLabel(wasteVolume),
      preferredDate: pickupType === 'special' ? preferredDate : null,
      timeSlot: pickupType === 'special' ? timeSlot : null,
    };
        ...
        <div className="container" style={{display:'flex',gap:20,alignItems:'flex-start'}}>
          ...
          <div style={{flex:1}}>
            ...
            <div className="card" style={{marginBottom:24, background:'#fff', padding:'32px'}}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{display:'flex', gap:12, justifyContent:'flex-start'}}>
                  <button
                    type="submit"
                    style={{padding:'12px 32px', background:'#10B981', color:'#fff', border:'none', borderRadius:10, fontWeight:700, fontSize:15, cursor:'pointer', boxShadow:'0 4px 12px rgba(16,185,129,0.3)', transition:'all 0.2s'}}
                  ...
                  <button
                    type="button"
                    style={{padding:'12px 24px', background:'#fff', color:'var(--text-700)', border:'1px solid #E5E7EB', borderRadius:10, fontWeight:600, fontSize:15, cursor:'pointer', transition:'all 0.2s'}}
            ...
            <div className="card" style={{background:'#fff', padding:'32px'}}>
              ...
              {/* Empty State - will be replaced with actual requests from backend */}
              ...
              {/* Placeholder for future requests list - will be populated from backend */}
              {/* 
              <div style={{display:'flex', flexDirection:'column', gap:16}}>
                {pickupRequests.map((request) => (
                  <div key={request.id} style={{display:'flex', alignItems:'center', gap:16, padding:16, borderRadius:12, background:request.bgColor, border:`1px solid ${request.borderColor}`}}>
                    <div style={{flexShrink:0}}>
                      {request.icon}
                    </div>
                    <div style={{flex:1}}>
                      <div style={{fontWeight:700, fontSize:15, color:'var(--text-900)', marginBottom:4}}>{request.title}</div>
                      <div style={{fontSize:13, color:'var(--text-600)'}}>{request.details}</div>
                    </div>
                    <StatusPill color={request.statusColor}>{request.status}</StatusPill>
                  </div>
                ))}
              </div>
              */}
